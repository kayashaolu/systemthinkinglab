...
Add the Systems Thinking Lab marketplace once, then install:
...
Then type `/design-with-blocks` to invoke. Updates later are just
`/plugin marketplace update systemthinkinglab`.

### Option B: Claude.ai Project

1. Create a new Project at claude.ai.
2. Upload every file in `skills/design-with-blocks/knowledge/` as Project Knowledge.
